...
3. To run the server API:
   ...
   3. Create the database with `pnpm db:generate`.
   4. Migrate the database with `pnpm migrate`.
   5. Seed the database with `pnpm db:seed`.
   6. Run the server with `pnpm dev`. This will start the server on `http://localhost:3000`.
...
4. To run the application:
 ...
 notes: if error do not log in or register
 access any page automatic redirect back to login page
